...
RE_TYPE_IN_NAME = re.compile(
    r'(?P<type>COTIS|VIR|VIREMENT|EVI|PRLV SEPA|ECHEANCE PRET|CHEQUE|RET DAB|COMMISSION FACTURETTE CB)'
    r'($| (?P<name>.*))')
...
def fix_transaction_type_from_name(transac, stmttrnrs):
    ...
    if match_type_in_name:
        transac.trntype.data = {
            ...
            'CHEQUE': 'CHECK',
            'RET DAB': 'ATM',
            'COMMISSION FACTURETTE CB': 'SRVCHG',
        ...
        if match_type_in_name.group('type') == 'COMMISSION FACTURETTE CB':
...
def fix_check_deposit(transac):
    ...
    if match_depot_cheque:
        transac.trntype.data = 'CHECK'
        transac.memo = transac.name
        transac.name.data = 'CHEQUE'
